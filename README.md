# SolaX Inverter - Home Assistant Picture Elements Card

Vlastní konfigurace karty **Picture Elements** pro komplexní monitoring FVE systému SolaX v Home Assistantovi. Karta zobrazuje toky energie, stavy baterie, teploty a detailní data po fázích.
...
- **Baterie:** Grafické zobrazení, procenta (SOC), výkon (W), teplota, kapacita.
- **Střídač:** Aktuální AC výkon, teploty (vnitřní/vnější), status, sériové číslo.
- **Monitoring fází:** Napětí (V), Proud (A) a Výkon (W) pro L1, L2, L3.
- **Bezpečnost:** Integrace kouřového senzoru a teploty/vlhkosti v technické místnosti.
- **Ovládání:** Tlačítko pro manuální spuštění chlazení střídače.

## 🛠️ Požadavky

1. **Obrázek pozadí:** Soubor `FVE_solax1.png` nahrajte do složky `/config/www/` ve vašem Home Assistantovi.
2. **Integrace:** Funkční integrace SolaX (Modbus) a vytvořené senzory dle konfigurace níže.

## 💻 YAML Konfigurace

Vložte tento kód do **Manuální karty** (Manual card) ve vašem Lovelace dashboardu:
